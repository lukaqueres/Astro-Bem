...
class UserManageController extends Controller {
    ...
    public function add(Request $request)
    {
        if ($request->missing('name') or $request->input('name') == '' ) {
            return back()->with('newUserStatus', 'Name is required, please input user\'s name.')->onlyInput($request->except('password'));
        }
        if ($request->missing('email') or $request->input('email') == '' ) {
            return back()->with('newUserStatus', 'Email not provided, please input users\'s email.')->onlyInput($request->except('password'));
        }
        if ($request->missing('password') or $request->input('password') == '' ) {
            return back()->with('newUserStatus', 'Password not provided, please input users\'s password.')->onlyInput($request->except('password'));
        }
        $users = User::all();
        foreach ( $users as $u ) { // - Check if id is always unique -
            if ( $request->input('email') == $u->email ) {
                ...
                ])->onlyInput($request->except('password'));
        ...
        return back()->with('newUserStatus', 'User created.')->onlyInput($request->except('password'));
    }
}
